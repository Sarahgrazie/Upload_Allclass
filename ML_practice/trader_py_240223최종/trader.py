# 모듈 import
# 접속 모듈
from connect import eBESTConnect, ExecuteSQL
# API 공통 모듈
...
class AutoTrade(eBESTAPI):
    ...
    def __init__(self, access_token):
        ...
        # 매수 주문
        self._buy_order = self.auto_buy_order(self.all_ticker_list)
        # 매도 주문
        self._sell_order = self.auto_sell_order(self._sell_list)
        # 트레이드 함수 실행
        self.auto_trade = self.run_trade_schedule()
        # 결산 실행
        self.day_closing = DayClosing(self.access_token)
        # 일일 매매일지
        self.t0150_df = self.day_closing.t0150_df
        # 일일 최종 잔고
        self.t0424_df = self.day_closing.t0424_df
    ...
    def run_trade_schedule(self):
        ...
        """
        # 자동주문 실행주기 설정 : 1분
        job_buy_order = schedule.every(1).minutes.do(self.auto_buy_order, self.all_ticker_list)
...
# OPEN API 키 설정
APP_KEY = 'PSN3INKOr8Un87KTS4dp9AW8Ol9g2I28kCEU'
APP_SECRET = 'WbCYqdEuMbOHqwc4jaWVwMTF0CX1ZYiG'
...
# 자동매매 실행
auto_trade = AutoTrade(ACCESS_TOKEN)
